...
import ModalNewImageFotogallery from "./ModalNewImageFotoGallery";
import ModalViewFotoGallery from "./ModalViewFotoGallery";

SwiperCore.use([Lazy, Navigation, Pagination]);

function SwiperFotoGallery(props) {
  ...
  return (
    <div className="flex flex-col relative bg-white">
      <h1 className="text-base text-left border-b pb-1">Foto Galery :</h1>
      <div className="relative mb-5 text-center h-34 px-3">
        {data && data?.status ? (
          data?.results.listgallery.length === 0 ? (
            <div className="h-36 flex items-center justify-center border-b">
              ...
              >
                ...
                <p>Klik untuk menambah Foto Gallery</p>
              </div>
            </div>
          ) : (
            <Swiper
              ...
              // slide
              slidesPerView={4}
              initialSlide={data?.results?.listgallery.length - 1}
              centeredSlides={true}
              ...
              className="border-b"
            >
              {data?.results.listgallery.map((slideContent, index) => (
              ...
              <SwiperSlide
                key={data?.results.listgallery.length}
                virtualIndex={data?.results.listgallery.length}
              >
                <div
      ...
      {/* Modal */}
      <ModalNewImageFotogallery
        open={newFoto}
        data={data} //data
        close={() => setNewFoto(false)}
      />
      {viewFoto && (
        <ModalViewFotoGallery
...
export default SwiperFotoGallery;
