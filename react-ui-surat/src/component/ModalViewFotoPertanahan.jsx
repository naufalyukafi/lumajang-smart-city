...
export default function ModalViewFotoPertanahan(props) {
  const { data } = props;
  ...
  const createProcess = (values) => {
    ...
    const reqUpdate = axios.put(
      `${API.HOST}/pertanahan/foto/update/${data.id}`,
      {
        id: data.id,
        type: values.caption,
      },
    ...
    swal({
      ...
      switch (value) {
        case "okee":
          ...
          setIsLoad(true);
          const callupload = axios.delete(`${API.HOST}/pertanahan/foto/deleted`, {
            headers: {
              Authorization: "Bearer " + localStorage.getItem("xtoken"),
            },
            timeout: 1000 * 70,
