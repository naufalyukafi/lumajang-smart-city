...
const maxSizePertanahan = 5 * 1024 * 1024;
const { fileuploaded, Sequelize } = require("../models");
...
const storagePertanahan = multer.diskStorage({
  //Specify the destination directory where the file needs to be saved
  destination: function (req, file, cb) {
    cb(null, "./assets/images/pertanahan/upload");
  },
  //Specify the name of the file. The date is prefixed to avoid overwriting of files.
  filename: function (req, file, cb) {
    let extArray = file.mimetype.split("/");
...
const uploadPertanahan = multer({
  storage: storagePertanahan,
  fileFilter: (req, file, cb) => {
    if (
      file.mimetype == "image/png" ||
      file.mimetype == "image/jpg" ||
  ...
  limits: { fileSize: maxSizePertanahan },
});
...
exports.uploadsinglePertanahan = () => {
  return function (req, res, next) {
    uploadPertanahan.single("photo")(req, res, async (err) => {
      // call as a normal function
      if (err) {
        const merr =
          err.message === "File too large"
